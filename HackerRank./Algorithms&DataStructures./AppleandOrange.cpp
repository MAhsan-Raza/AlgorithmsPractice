...
void countApplesAndOranges(int startHouse, int endHouse, int aplTree, int orgTree,  vector<int> apples, vector<int> oranges) 
{
    long nAplz = 0;
    long nOrgz = 0;
    ...
    {
        long dist = ((long)aplTree + (long)x);
        if(dist >= startHouse && dist <= endHouse)
            ++nAplz;
    }
    cout << nAplz << endl;

    for(const auto& y : oranges)
    {
        long dist = ((long)orgTree + (long)y);
        if(dist <= endHouse && dist >= startHouse )
            ++nOrgz;
    }
    cout << nOrgz << endl;
}
